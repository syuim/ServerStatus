...
void CMain::OnNewClient(int ClientNetID, int ClientID)
{
	...
	net_addr_str(m_Server.Network()->ClientAddr(ClientNetID), Client(ClientID)->m_aIP, sizeof(Client(ClientID)->m_aIP), false);
	...
	while(gs_Running)
	{
		...
		for(int i = 0; i < NET_MAX_CLIENTS; i++)
		{
			...
			if(pClients[i].m_Connected)
			{
				...
				// custom 是原始 JSON 字符串，需转义引号/反斜杠以保证 stats.json 整体合法
				char aEscapedCustom[1024];
				const char *pSrc = pClients[i].m_Stats.m_aCustom;
				char *pDst = aEscapedCustom;
				for(; *pSrc && pDst - aEscapedCustom < (int)sizeof(aEscapedCustom) - 4; pSrc++)
				{
					if(*pSrc == '"' || *pSrc == '\\')
						*pDst++ = '\\';
					*pDst++ = *pSrc;
				...
				*pDst = 0;
				...
				str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"name\": \"%s\", \"type\": \"%s\", \"host\": \"%s\", \"location\": \"%s\", \"online4\": %s, \"online6\": %s, \"uptime\": \"%s\", \"load\": %.2f, \"network_rx\": %" PRId64 ", \"network_tx\": %" PRId64 ", \"network_in\": %" PRId64 ", \"network_out\": %" PRId64 ", \"cpu\": %d, \"memory_total\": %" PRId64 ", \"memory_used\": %" PRId64 ", \"swap_total\": %" PRId64 ", \"swap_used\": %" PRId64 ", \"hdd_total\": %" PRId64 ", \"hdd_used\": %" PRId64 ", \"custom\": \"%s\" ,\"region\": \"%s\"},\n",
					pClients[i].m_aName, pClients[i].m_aType, pClients[i].m_aIP, pClients[i].m_aLocation, pClients[i].m_Stats.m_Online4 ? "true" : "false", pClients[i].m_Stats.m_Online6 ? "true" : "false",	aUptime, pClients[i].m_Stats.m_Load, pClients[i].m_Stats.m_NetworkRx, pClients[i].m_Stats.m_NetworkTx, pClients[i].m_Stats.m_NetworkIN, pClients[i].m_Stats.m_NetworkOUT, (int)pClients[i].m_Stats.m_CPU, pClients[i].m_Stats.m_MemTotal, pClients[i].m_Stats.m_MemUsed, pClients[i].m_Stats.m_SwapTotal, pClients[i].m_Stats.m_SwapUsed, pClients[i].m_Stats.m_HDDTotal, pClients[i].m_Stats.m_HDDUsed, aEscapedCustom, pClients[i].m_aRegion);
				pBuf += strlen(pBuf);
			}
			else
			{
